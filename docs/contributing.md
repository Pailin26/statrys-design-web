...
- **Core DS** — [statrys-design-web](https://github.com/Pailin26/statrys-design-web)
  ...
  shape* should exist on both platforms where possible (see `manifest.json`'s
  `platform` field to tell them apart) — check the other repo when adding a
...
2. Add `component/<name>.json` in the **statrys-tokens** repo — see "Component-level
   tokens" below. Do this **before** writing the component's styles, not after.
   Push it there first, then bump this repo's `@statrys/tokens` dependency to
   pick it up (see "Adding or changing a token").
...
in exactly one place: statrys-tokens' `component/<name>.json`. The
...
another component's or a shared `semantic/` file's tokens directly either. If
a component needs a shared semantic value (e.g. `text.neutralInverse.primary`),
alias it into the component's own file first (see `button.textOnFill` in
`component/button.json` for the pattern) rather than referencing `text.*` from
component code.
...
primitive (brand.5, radius.md, fontSize.base, ...)
   ↓ referenced by
component/<name>.json   (button.primary, button.radius, button.fontSizeMd, ...)
   ↓ built into
...
Change a primitive's value, or a component's semantic mapping, in the
**statrys-tokens** repo, run `npm run build` there, then bump this repo's
`@statrys/tokens` git dependency (`npm install` re-runs its `prepare` script)
and rebuild the consuming app — every component using that token updates. No
component code changes here, because component code never held the value in
the first place.

statrys-tokens' `build/build.js` (not just `build/config.json`) registers two
...
`@statrys/tokens/fonts/GTWalsheimLC-*.ttf` — not the `.woff2` web-playground
...
1. Primitive values go in `primitives/`, cross-cutting semantic mappings in
   `semantic/`, per-component mappings in `component/`
2. Never hand-edit `dist/` — generated by `npm run build`
3. Note in the PR which platform(s)/products might be affected
4. Once merged, bump `@statrys/tokens` in the consuming repo(s) (`web-ds`/`app-ds`,
   and anywhere else that depends on it directly, like a playground's Foundation
   page) and rebuild
...
statrys-tokens' `fonts/`, for the two platforms' different loading
mechanisms:
- `*.woff2` — copied from accounting's own `public/fonts/`, loaded via
  `@statrys/tokens/fonts.css` on web.
...
- [ ] Component style file references only `component/<name>.json`
